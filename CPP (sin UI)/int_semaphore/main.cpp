...
void initializeGrid(vector<vector<int>> *currentGrid, vector<vector<int>> *nextGrid, bool starter_cell_is_STC, int cell_proliferation_potential_max)
{
    ::size = 400;
    currentGrid->assign(::size, vector<int>(::size, 0));
    nextGrid->assign(::size, vector<int>(::size, 0));
    if (starter_cell_is_STC)
    {
        (*currentGrid)[::size / 2][::size / 2] = cell_proliferation_potential_max + 1;
        (*nextGrid)[::size / 2][::size / 2] = cell_proliferation_potential_max + 1;
    }
    else
    {
        (*currentGrid)[::size / 2][::size / 2] = cell_proliferation_potential_max;
        (*nextGrid)[::size / 2][::size / 2] = cell_proliferation_potential_max;
    }
}
...
int main()
{
    ...
    vector<vector<int>> nextGrid(::size, vector<int>(::size, 0));
